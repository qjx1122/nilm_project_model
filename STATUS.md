...
## 当前目标
- [x] 已完成（2026-09-05 攻坚）：**F10 定稿为新稳定版**（MAE 6.68W/R² .770/EE −10.0%/P .938/R .797/F1 .862，详见 REPORT.md §4）。双 0.9 判定：500W 点级协议被标签噪声锁定（FP 76% 子电表丢数/FN 29% 不可学/跨模型重合 98%）；median4 集成 test 最优运营点 .906/.906、val 选点 .907/.880——差 val 分辨率(80 ON 点)，遗留项含 val 加密复评与口径重议。[原任务描述↓]：稠密 test30000 上 P>0.9 且 R>0.9（硬约束，500W 判定）；MAE 0.2W 口径经确认「物理不可达需重设」→ 目标改为尽量压低（≤6W）。泳道：F6(boost-only) → F7(stochastic boost+权重平均) → **F8(容量×4+d128/4L+stochastic+wavg，跑中，ETA≈10:3x)**。
...
## 进行中
- （无——P/R 攻坚专题闭环：F10 定稿新稳定版，双 0.9 判定为标签噪声锁死并出具完整定量依据，全部过程见 REPORT_TEST.md 末节与 REPORT.md §3/§4）

## 下一步（TODO）
1. **多 seed 复验补全**：F10 主结论单 seed（seed43 的 F13：回归面稳、运营点面 ±2pp），seed44 未跑；决策层选点用 **val 加密到 30k** 再复评（当前 val 8k 仅 ~80 ON 点，分辨率 0.012 是 .880 vs .906 差异的直接原因）
2. **roll_jitter 全长单变量对照**（F8 配方 + jitter only，未跑过；F12 是热启动 12ep 版，双刃定性可能低估其全长潜力）
3. **500W 双 0.9 口径重议（需用户决策）**：可选（i）决策阈值运营点 95-115W（median4 已达 .906/.906）、（ii）事件级 P/R 口径、（iii）数据修复（子电表时钟对齐/丢数修补）；三条路的定量依据均已在 REPORT_TEST 末节
...
## 决策记录 / 踩坑
- [2026-09-07 评估] 生产就绪三条件判定：median4 运营点 F1/R/SAE=​.9060/.9060/.189 点估计全过、统计待 B3 转正；F10 严格口径不达标(维持锁定结论)。结论=可灰度不可正式验收；能量标定建议已给出(零风险修 SAE 余量)。全文见 REPORT_TEST 评估专题。
...
- [2026-09-07 文档] 新增 `TUNING_GUIDE.md`（面向无算法背景工程师的调参教学：指标人话版/配置字典/五铁律/26 轮战史/SOP/命令/坑单/术语表），已登记入 BOOTSTRAP 文档表（更新规则=整篇重写对账，不追加流水）；README 顶部加阅读路径、文件树补登；指南数字全部对照 CSV/A1 产物核定稿（修正三处：99.37% 表述、drop0 收益口径、攻坚尸检 FN/FP 实数 54/14 并弃用 665 旧估）。
- [2026-09-07 A1 结果] val 加密 30k+max-min 选点：median4 t*=95 → test P/R/F1=.9060/SAE .189，四元约束运营点口径点估计达标（选点缺口 0.026 被精确收复）；B1 滤波被点标签机制作废（判死）；bootstrap CI [.868,.940]→按预注册记'达标待复验'，转正=B3 补 seed44/45 或 C1 剔噪。修正：N_ON 实为 266（前案 665 系比率反推高估）。
