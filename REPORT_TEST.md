...
## [2026-09-07] 专题：生产就绪评估（F1>0.9 ∧ R>0.9 ∧ SAE<0.2 三条件）
- 类型：评估专题（零训练；数据全部来自 a1_val30k.json / tuning_rounds.csv，能量标定为估算值并注明）
- 判定：①稳定版 tune_f10_biglong@严格500W双端：F1 .862✗ / R .797✗ / SAE .100✓ → 不达标（该口径不可达结论不变）；其运营点(t*=110,val选)：F1 .903✓/R .891✗(差0.9pp)/SAE .100✓。②median4@运营点t*=95(val30k选点)：F1 .9060✓/R .9060✓/SAE .189✓ → **点估计 3/3 达标**；但 n_ON=266、1σ=±1.84pp，bootstrap min(P,R) CI=[.868,.940]（P(≥.90)≈.63）→ 按预注册记"达标待复验"。未列 P 约束的隐含线：R=.906∧F1>.9 ⇒ P>.894，实测 .906 余量 .012。
- 生产结论：**可有条件上线（灰度），不可作正式验收**。三项保留：统计未转正、口径为运营点（预测线95W≠标签500W，写进 SLA）、SAE 预算余量仅 .011。非指标面：推理单窗~1.5ms/单模型（A1 实测 8×30k 窗≈24min，2核）实时余量巨大，median4=4× 成本可蒸馏单模型；跨户泛化未验证（仅 House1 kettle）；在线 P 监控须用事件级/抽核口径（76% FP 为子表丢数会低估精度）；归一化统计冻结自训练段，长期上线需漂移监控。
- 下一步方向（序）：B3 seed44/45 补员 median6（4-5h，转正统计结论，池化 σ→1.1pp）；能量标定（val-only scale≈1.23，单调、P/R 零影响，SAE→.02-.08）；P 余量薄则改 trimmed/top-2 聚合；R 增量走 B2+事件级口径或 C1 数据修复；验收若锁 500W 双端则维持"不可达、需重议口径"判定。
